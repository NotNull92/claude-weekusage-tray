...
3. `ClaudeWeekUsageTray.exe`를 실행합니다.
   ...
   설정이 되어 있는지 확인하고 안 되어 있으면 물어봅니다.

   <img src="docs/first-run.png" width="404" alt="Claude Code 연결을 제안하는 첫 실행 창">

   **Connect**를 누르면 설정 파일을 백업한 뒤 대신 써 줍니다. 이미 쓰시던
   상태 표시줄 명령이 있으면 먼저 알려주고, 그 명령은 그대로 실행됩니다.
...
Claude Code가 실행할 경로를 저장해 두므로 폴더를 옮기면 연결이 끊깁니다.
옮긴 위치에서 실행하면 그 사실을 알아채고 새 위치로 바꿀지 물어봅니다. 새
릴리스를 다른 폴더에 풀었을 때도 같은 창이 뜹니다. 다만 **실행 중인 복사본을
메뉴의 Exit로 먼저 종료**해야 합니다. 그러지 않으면 새 복사본은 창을 띄우지
않고 실행 중인 쪽의 패널만 엽니다.
